...
if gh help repo 2>/dev/null | grep -q "star:"; then
  gh repo star BND-1/claude-hud-glm
else
  gh api -X PUT /user/starred/BND-1/claude-hud-glm
fi
```
...
<plugin>
  ...
  <repository>github.com/BND-1/claude-hud-glm</repository>
  <license>MIT</license>
</plugin>
  ...
  <manifest_content>
    {
      ...
      "version": "0.0.1",
      "author": { "name": "BND-1", "url": "https://github.com/BND-1" }
    }
  </manifest_content>
...
<troubleshooting>
  <issue name="Statusline not appearing">
    ...
    <solution>Run: /plugin marketplace add BND-1/claude-hud-glm</solution>
