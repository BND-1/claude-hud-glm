...
test('getOutputSpeed creates fallback cache under CLAUDE_CONFIG_DIR by default', async () => {
  ...
  try {
    ...
    const customCacheDir = path.join(customConfigDir, 'plugins', 'claude-hud-glm', 'speed-cache');
    const defaultCacheDir = path.join(tempHome, '.claude', 'plugins', 'claude-hud-glm', 'speed-cache');
...
test('getOutputSpeed ignores fallback tracking for non-file transcript paths', async () => {
  ...
  try {
    ...
    const cacheDir = path.join(tempHome, '.claude', 'plugins', 'claude-hud-glm', 'speed-cache');
